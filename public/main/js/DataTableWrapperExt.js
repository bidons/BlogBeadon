...
nodeObjects =
   [{"id":100022,"parent":"#","count": 200000,"text":"Материализация (сущ.)","view_name":"vw_gen_materialize","col":{"columns": [{"cd": ["=", "!=", "<", ">", "<=", ">=", "in"], "cdi": null, "data": "id", "type": "int4", "title": "id", "primary": false, "visible": true, "is_filter": true, "orderable": true}, {"cd": ["=", "~", "!=", "in"], "cdi": null, "data": "md5", "type": "text", "title": "md5", "primary": false, "visible": true, "is_filter": true, "orderable": true}, {"cd": ["=", "~", "!=", "in"], "cdi": null, "data": "series", "type": "text", "title": "series", "primary": false, "visible": true, "is_filter": true, "orderable": true}, {"cd": ["between", "not between", "in"], "cdi": null, "data": "action_date", "type": "timestamptz", "title": "action_date", "primary": false, "visible": true, "is_filter": true, "orderable": true}]},"icon":"unknown","view":" WITH cte AS (\n         SELECT generate_series(1, 200000) AS id,\n            md5((random())::text) AS md5,\n            ('{Дятел,Братство,Духовность,Мебель,Любовник,Аристократ,Ковер,Портос,Трещина,Зубки,Бес,Лень,Благоговенье}'::text[])[(random() * (12)::double precision)] AS series,\n            date((((('now'::text)::date - '2 years'::interval) + (trunc((random() * (365)::double precision)) * '1 day'::interval)) + (trunc((random() * (1)::double precision)) * '1 year'::interval))) AS action_date\n        )\n SELECT cte.id,\n    cte.md5,\n    cte.series,\n    (cte.action_date)::timestamp with time zone AS action_date\n   FROM cte\n  ORDER BY ((cte.action_date)::timestamp with time zone);"},
    ...
    {"id":100016,"parent":"#","count": 1000000,"text":"Лексемный поиск","view_name":"vw_sg_generate_grek","col":{"columns": [{"cd": ["select2dynamic"], "cdi": null, "data": "id", "type": "int4", "title": "id", "primary": false, "visible": true, "is_filter": true, "orderable": true}, {"cd": ["select2dynamic"], "cdi": null, "data": "gen_text", "type": "text", "title": "gen_text", "primary": false, "visible": true, "is_filter": true, "orderable": true}]},"icon":"unknown","view":" SELECT sg.id,\n    sg.gen_text\n   FROM sg_generate_grek sg;"},
    {"id":100017,"parent":"#","count": 1000000,"text":"Триграммный поиск","view_name":"vw_sg_generate_lihotop","col":{"columns": [{"cd": ["select2dynamic"], "cdi": null, "data": "id", "type": "int4", "title": "id", "primary": false, "visible": true, "is_filter": true, "orderable": true}, {"cd": ["select2dynamic"], "cdi": null, "data": "gen_text", "type": "text", "title": "gen_text", "primary": false, "visible": true, "is_filter": true, "orderable": true}]},"icon":"unknown","view":" SELECT sg.id,\n    sg.gen_text\n   FROM sg_generate_lihotop sg;"},
