...
class FixPagingTable extends AbstractMigration
{
    /**
     * Change Method.
     *
...
CREATE OR REPLACE FUNCTION materialize_worker(a_mode text, a_object text, a_person_owner integer) returns jsonb
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  -- Генерируем ошибку если не определена обвёртка или не являеится материализованной
  PERFORM message_ex(val_is_mat IS FALSE or val_paging_table_id IS NULL, 'Materialize view not found: "' || a_object || '"', -1);
